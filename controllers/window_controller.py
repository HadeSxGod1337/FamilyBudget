...
class WindowController2:
    ...
    def add_category(self):
        category = self.main.categories.currentText()
        print("Добавление категории")
        if self.db_controller.add_category(category):
            show_message("Добавление категории", f"Категория {category} успешно добавлена")
    ...
    def update_table(self):
        try:
            result = self.db_controller.get_choose_transactions(self.main.choose_user.currentText(),
                                                                self.main.choose_type.currentText(),
                                                                self.main.choose_categoty.currentText())
